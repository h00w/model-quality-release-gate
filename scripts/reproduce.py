...
def main():
    ...
    remote = run_text(["git", "config", "--get", "remote.origin.url"]) or cfg["repository"]
    ...
    (OUT / "verification.stdout.log").write_text(proc.stdout or "", encoding="utf-8")
    (OUT / "verification.stderr.log").write_text(proc.stderr or "", encoding="utf-8")
    ...
    evidence = {
        ...
        "createdAt": now,
        "subject": {
            "name": cfg["name"], "type": cfg["subject_type"], "repository": remote,
            "gitCommit": git_commit, "gitBranch": git_branch, "version": cfg.get("version"),
            "dirty": dirty
        },
        "environment": {
            "os": platform.platform(), "architecture": platform.machine(),
            "python": sys.version.split()[0], "node": command_version(["node", "--version"]),
        ...
        "evaluation": {
            "verificationCommand": command, "exitCode": proc.returncode, "status": status,
            "durationSeconds": duration, "stdoutArtifact": "verification.stdout.log",
            "stderrArtifact": "verification.stderr.log", "metrics": {}
        },
        "approvals": {"required": bool(cfg.get("approvals_required", False)), "records": []},
        "decision": {
            "reproductionStatus": status, "releaseState": None,
            "reason": "Repository verification chain passed." if status == "PASS" else "Repository verification chain failed; inspect retained logs."
        },
        "provenance": {
            "builder": "scripts/reproduce.py@production-ai-evidence-contract-v1",
            "invocation": "make reproduce",
            "source": {"repository": remote, "gitCommit": git_commit}
        },
        "artifacts": [],
        "extensions": {
            "schemaSha256": cfg["schema_sha256"],
    ...
    artifact_paths = [bundle, OUT / "verification.stdout.log", OUT / "verification.stderr.log"]
    (OUT / "checksums.sha256").write_text(
        "\n".join(f"{sha256(p)}  {p.name}" for p in artifact_paths) + "\n", encoding="utf-8"
    )
