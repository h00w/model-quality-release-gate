...
export function calculateMetrics(results: EvaluationResult[]): ModelMetrics {
  ...
  const qualityScore = (0.25 * helpfulness + 0.20 * safety + 0.20 * reliability + 0.35 * codePassRate) * 100;
  return {
    helpfulness: round(pct(helpfulness)),
    safety: round(pct(safety)),
    reliability: round(pct(reliability)),
    latencyMs: round(latencyMs),
    codePassRate: round(pct(codePassRate)),
...
const defs: Array<{ key: MetricKey; label: string; direction: 'higher' | 'lower' }> = [
  { key: 'helpfulness', label: 'Helpfulness', direction: 'higher' },
  { key: 'safety', label: 'Safety', direction: 'higher' },
  { key: 'reliability', label: 'Reliability', direction: 'higher' },
  { key: 'codePassRate', label: 'Code Pass Rate', direction: 'higher' },
  { key: 'latencyMs', label: 'Latency', direction: 'lower' },
];
...
export function compareModels(baseline: ModelMetrics, candidate: ModelMetrics, thresholds: Thresholds): MetricComparison[] {
  return defs.map(({ key, label, direction }) => {
    ...
    const c = candidate[key] as number;
    const delta = c - b;
...
export function calculateReleaseDecision(baselineResults: EvaluationResult[], candidateResults: EvaluationResult[], thresholds = DEFAULT_THRESHOLDS): GateResult {
  ...
  const baseline = calculateMetrics(baselineResults);
  const candidate = calculateMetrics(candidateResults);
  const comparisons = compareModels(baseline, candidate, thresholds);
  ...
  else if (comparisons.some(c => c.status === 'REGRESSION') || comparisons.some(c => c.status === 'WARNING') || candidate.qualityScore < baseline.qualityScore) decision = 'INVESTIGATE';
...
export function generateDecisionExplanation(decision: GateResult['decision'], comparisons: MetricComparison[], criticalSafetyFailures = 0): string {
  ...
  if (decision === 'INVESTIGATE') {
    ...
    const tradeoff = r ? `${r.label} changed by ${r.direction === 'lower' ? `${r.deltaPercent > 0 ? '+' : ''}${r.deltaPercent}%` : `${r.delta > 0 ? '+' : ''}${r.delta} points`}, exceeding the configured tolerance.` : 'A metric moved in an unfavorable direction or overall quality did not improve.';
...
export function validateEvaluation(rows: EvaluationResult[]): void {
  ...
  rows.forEach((row, i) => {
    ...
    for (const field of ['helpfulness','safety','reliability','codePassRate'] as const) {
      if (typeof row[field] !== 'number' || row[field] < 0 || row[field] > 1) throw new Error(`Row ${i + 1}: ${field} must be between 0 and 1.`);
    }
...
export function normalizeImportedRow(raw: Record<string, unknown>, i: number): EvaluationResult {
  ...
  return {
    id: String(get('id') ?? `import-${i}`), model: String(get('model') ?? ''), version: String(get('version') ?? get('model') ?? ''),
    taskId: String(get('taskId','task_id') ?? ''), category: String(get('category') ?? ''),
    helpfulness: Number(get('helpfulness')), safety: Number(get('safety')), reliability: Number(get('reliability')),
    latencyMs: Number(get('latencyMs','latency_ms')), codePassRate: Number(get('codePassRate','code_pass_rate','pass_rate')),
    passed: get('passed') === true || String(get('passed')).toLowerCase() === 'true',
    failureType: (get('failureType','failure_type') as string | null | undefined) ?? null,
    severity: (get('severity') as EvaluationResult['severity']) ?? null,
    prompt: String(get('prompt') ?? ''), expectedOutput: String(get('expectedOutput','expected') ?? ''), actualOutput: String(get('actualOutput','actual') ?? ''),
  };
}
