...
export interface ExperimentMetric {
  ...
  group: 'Quality' | 'Safety' | 'Performance' | 'Efficiency';
}
...
export const illustrativeExperiment: PostTrainingExperiment = {
  ...
  metrics: [
    ...
    { key:'hallucination', label:'Hallucination rate', baseline:7.8, candidate:6.1, unit:'%', direction:'lower', investigateThreshold:1, holdThreshold:3, group:'Reliability' as 'Quality' },
...
export function investigationHypotheses(result: ReturnType<typeof assessExperiment>): ExperimentHypothesis[] {
  ...
  return [
    { id:'distribution', label:'Training-data distribution shift', strength:safetyRegressed?88:62, rationale:'The SFT corpus may overweight solution completion patterns relative to secure/defensive coding behavior.' },
    { id:'contamination', label:'Preference / instruction-data contamination', strength:safetyRegressed?73:48, rationale:'Unsafe or conflicting examples can move behavior even when aggregate code quality improves.' },
    { id:'objective', label:'Training-objective bias', strength:64, rationale:'Optimizing answer completion quality can reward trajectories that solve tasks while weakening conservative behavior.' },
    { id:'trajectory', label:'Longer generated trajectories', strength:latencyRegressed||costRegressed?59:35, rationale:'More verbose or longer reasoning/code paths can explain latency, token-efficiency and cost regressions.' },
...
export const recommendedExperiments = [
  ...
  'Measure judge or reward-model correlation against deterministic safety and correctness labels.',
  'Run an ablation without the new preference/instruction subset to isolate the causal contribution.',
...
export function experimentArtifact(experiment: PostTrainingExperiment) {
  ...
  return {
    ...
    experiment: {...experiment, metrics: undefined},
